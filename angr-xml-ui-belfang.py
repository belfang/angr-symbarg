...
import angr, claripy, time, sys, os, simuvex, struct
import ntpath
...
def get_test_case(s, dic_args, list_files, stdin_size, count):
    ...
    output = open("test_{}.bin".format(str(count)), "wb")

    output.write(struct.pack("i", len(dic_args)+len(list_files)))
...
def collect_angr_result(sm, dic_args, list_files, stdin_size):
    ...
    for s in sm.deadended:
        get_test_case(s, dic_args, list_files, stdin_size, count)
        count=count+1
    os.chdir("../")
